...
static RThreadFunctionRet thchan_handler(RThread *th) {
	eprintf ("the background thread starts here\n");
...
eprintf ("Message received! lets call core.cmd with it\n");
...
R_API void r_core_cmd_r(RCore *core, const char *cmd) {
	r_return_if_fail (core && cmd);
	// reentrant function, that needs to lock
#if 0
	if (!core->chan_thread) {
		core->chan = r_th_channel_new ();
...
eprintf ("RUN (%s)%c", cmd, 10);
	// start a thread that processes all the calls to corecmdstrblah
	RThreadChannelMessage *msg = r_th_channel_message_new ((const ut8*)cmd, strlen (cmd) + 1);
	RThreadChannelMessage *res = r_th_channel_write (core->chan, msg);
	// RThreadChannelMessage *res = r_th_channel_message_read (core->chan, response);
	if (msg) {
